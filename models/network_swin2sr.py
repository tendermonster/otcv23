...
class Mlp(nn.Module):
    ...
    This class represents a multi-layer perceptron neural network module. It consists of two linear layers
    with an activation function and dropout applied between them.
    """
    def __init__(
        self,
        in_features,
        ...
        Args:
            ...
            hidden_features (int, optional): Number of hidden layer features. Defaults to None, which sets it equal to in_features.
            out_features (int, optional): Number of output features. Defaults to None, which sets it equal to in_features.
            act_layer (torch.nn.Module, optional): Activation function applied after the first linear layer. Defaults to torch.nn.GELU.
            drop (float, optional): Dropout probability applied after each linear layer. Defaults to 0.0.
        """
        super().__init__()
...
def window_partition(x, window_size):
    ...
    Returns:
        torch.Tensor: Partitioned windows tensor of shape (num_windows*B, window_size, window_size, C).

    Raises:
        ValueError: If the input tensor dimensions are incompatible.

    Notes:
        The function divides the input tensor into non-overlapping windows of the specified window size.
        The resulting windows tensor has the shape (num_windows*B, window_size, window_size, C),
        where num_windows is the number of windows that can be created from the input tensor.
...
def window_reverse(windows, window_size, H, W):
    """
    Reconstructs the original image from the window partitions.

    Args:
        windows (torch.Tensor): Window partitions tensor of shape (num_windows*B, window_size, window_size, C).
        window_size (int): Window size.
        H (int): Height of the image.
        W (int): Width of the image.

    Returns:
        torch.Tensor: Reconstructed image tensor of shape (B, H, W, C).

    Raises:
        ValueError: If the dimensions of the input tensor are incompatible.

    Notes:
        The function reconstructs the original image by reversing the window partitioning process.
        It takes a tensor of window partitions and reconstructs the original image tensor.
        The resulting tensor has the shape (B, H, W, C), where B is the batch size, H and W are the height and width of the image, and C is the number of channels.

    Example:
        >>> windows = torch.randn(16, 8, 8, 3)
        >>> image = window_reverse(windows, window_size=8, H=32, W=32)
    """
...
class UpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, input_resolution=None):
        """
        Initialize the UpsampleOneStep module.

        Args:
            scale (int): Scale factor. Supported scales: 2^n and 3.
            num_feat (int): Channel number of intermediate features.
            num_out_ch (int): Number of output channels.
            input_resolution (tuple): Input resolution (Height, Width).
        """
    ...
    def flops(self):
        """
        Calculates the number of floating-point operations (FLOPs) for the UpsampleOneStep module.

        Returns:
            int: Number of FLOPs.

        Notes:
            The FLOPs are calculated based on the input resolution and the number of intermediate features.
            It assumes that the input tensor has the same resolution as the specified input_resolution.
        """
        H, W = self.input_resolution
...
class Swin2SR(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsampler == "pixelshuffle":
            ...
            x = self.conv_last(
                x
            )  # nn.Conv2d(64, 3, 3, 1, 1) # high quality image reconstruction
